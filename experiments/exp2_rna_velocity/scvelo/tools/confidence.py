...
def get_indices(dist, n_neighbors=None, mode_neighbors="distances"):
    ...
    n_counts = np.asarray((D > 0).sum(axis=1)).flatten()
    n_neighbors = (
...
def inner_cluster_coh(adata, k_cluster, k_velocity, return_raw=False):
    ...
    """
    # Ensure neighbor indices exist, computing them if necessary
    if 'indices' not in adata.uns["neighbors"]:
        # Use scVelo's utility to properly extract indices from distances matrix
        indices, _ = get_indices(dist=get_neighs(adata, "distances"))
