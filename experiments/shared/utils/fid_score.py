...
from experiments.shared.utils.inception import InceptionV3
...
def calculate_inception_score(path, device=None, batch_size=50, splits=10, num_workers=8):
    """Calculates the Inception Score for a directory of images.
    ...
    model = tv_models.inception_v3(pretrained=True, transform_input=False).to(device)
    model.eval()
    ...
    path = pathlib.Path(path)
    files = sorted([f for ext in IMAGE_EXTENSIONS for f in path.glob(f'*.{ext}')])
