...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        * `x` has shape `[batch_size, in_channels, height, width]`
        ...
        # attn = attn.softmax(dim=2).clone()
        attn_max = attn.max(dim=2, keepdim=True)[0]
        attn = attn - attn_max
        
        # 2. Explicit Exp (creates new tensor 'exp_attn')
        exp_attn = torch.exp(attn)
        
        # 3. Explicit Division (creates new tensor 'attn_probs')
        #    We must ensure we do NOT modify exp_attn in place.
        sum_exp = exp_attn.sum(dim=2, keepdim=True)
        attn = exp_attn / sum_exp
