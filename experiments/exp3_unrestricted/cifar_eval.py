"""
Standalone DDP evaluation script for EDM on CIFAR-10.

Metrics computed:
  1. FID-50K  — generate 50K images with adaptive Heun-Euler ODE solver, compare to real stats
  2. NLL      — Hutchinson trace estimator on the 10K CIFAR-10 test set (bits/dim)
...
Usage:
  torchrun --nproc_per_node=4 cifar_eval.py \
      --config config/cifar_unconditional_EDM_flux.yaml \
...
def load_model(ckpt_path, opt, device, rank):
    ...
    # Load EMA on rank 0, broadcast weights to all ranks
    if rank == 0:
...
def compute_fid(model, image_size, device, fid_stats_path, rank, world_size,
    ...
    # Create one shared temp directory on rank 0 and broadcast the path.
    # This only works if the temp path is visible to all ranks.
    if rank == 0:
    ...
    try:
        ...
        while generated < my_n_samples:
            ...
            for i, img in enumerate(imgs):
                ...
                save_image(
                    img,
                    os.path.join(shared_tmp_dir, f"sample_{global_idx:06d}.png")
                )
        ...
        if rank == 0:
            ...
            is_mean, is_std = calculate_inception_score(shared_tmp_dir, device=device)

        # avg NFE per image: nfe counts drift() calls for the entire batch
        # (all images share the same ODE trajectory steps). Divide by number
        # of batches to get per-image NFE, then average across ranks.
        per_image_nfe = torch.tensor(
            total_nfe / num_batches, dtype=torch.float64, device=device
        )
        ...
        is_tensor = torch.tensor([is_mean, is_std], device=device)
        dist.broadcast(is_tensor, src=0)
        is_mean, is_std = is_tensor[0].item(), is_tensor[1].item()
    ...
    return fid, avg_nfe, is_mean, is_std
...
def compute_nll(model, test_set, device, rank, world_size, nll_batch_size,
    ...
    local_bpds = np.concatenate(local_bpds)  # [N_local]

    # Gather all bpds to rank 0
...
def main():
    ...
    if not opt.skip_fid:
        if rank == 0:
            print("\n=== FID-50K + Avg NFE ===")
        fid, avg_nfe, is_mean, is_std = compute_fid(
    ...
    # ------------------------------------------------------------------
    # 2: NLL on CIFAR-10 test set
    # ------------------------------------------------------------------
    if not opt.skip_nll:
        if rank == 0:
            print("\n=== NLL (bits/dim) on CIFAR-10 test set ===")
        tf = transforms.Compose([transforms.ToTensor()])
        test_set = CIFAR10("../shared/data", train=False, download=True, transform=tf)
    ...
    if rank == 0:
        ...
        with open(results_path, 'a') as f:
            ...
            if fid is not None:
                parts.append(f"fid50k={fid:.4f} avg_nfe={avg_nfe:.2f} IS={is_mean:.4f}±{is_std:.4f}")
            if nll_mean is not None:
