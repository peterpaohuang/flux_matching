...
def train(opt):
    ...
    if use_dsm: raise "Fast mixing is not supported with DSM"
    ...
    is_mixing = opt.mixing
    mixing_lam = opt.mixing_lam
    ...
    class Crop(object):
        def __call__(self, img):
            ...
            return F.crop(img, x1, y1, x2 - x1, y2 - y1)
    ...
    class CelebAUnlabeled(torch.utils.data.Dataset):
        def __init__(self, root, transform):
            tvdatasets.CelebA._check_integrity = lambda self: True
            self.base = tvdatasets.CelebA(root=root, split='train', target_type='attr', transform=transform, download=False)
        def __len__(self):
            return len(self.base)
        def __getitem__(self, i):
            return self.base[i][0]

    train_set = CelebAUnlabeled('../shared/data', tf)
    image_size = train_set[0].shape  # (C, H, W)

    print0("CelebA train dataset:", len(train_set))
    ...
    while step < opt.n_steps:
        ...
        if step % 50000 == 0 or step == opt.n_steps:
            ...
            # Broadcast EMA weights from rank 0 to all ranks
            if rank == 0:
                ema_sd = ema.ema_model.state_dict()
            else:
                ema_sd = copy.deepcopy(diff.module.state_dict())
