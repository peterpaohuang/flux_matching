import numpy as np
import torch
import torch.nn as nn
from tqdm import tqdm
from torch.cuda.amp import autocast as autocast

import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))

from src.loss import flux_matching_loss
from experiments.shared.baselines.dsm import dsm_loss_x0
from experiments.exp4_fast_mixing.application_loss import mixing_loss_fixed_sigma_edm

def normalize_to_neg_one_to_one(img):
    # [0.0, 1.0] -> [-1.0, 1.0]
    return img * 2 - 1


def unnormalize_to_zero_to_one(t):
    # [-1.0, 1.0] -> [0.0, 1.0]
    return (t + 1) * 0.5


class EDM(nn.Module):
    def __init__(self, nn_model, s_noise_normalizer,
                 sigma_data, p_mean, p_std,
                 sigma_min, sigma_max, rho,
                 S_min, S_max, S_noise,
                 device, importance_sampler_net=None, use_dsm=False,
                 mixing_noise_normalizer=None, mixing_lam=0.01):
        ''' EDM proposed by "Elucidating the Design Space of Diffusion-Based Generative Models".

            Args:
                nn_model: A network (e.g. UNet) which performs same-shape mapping.
                device: The CUDA device that tensors run on.
                geometric_network: Network that predicts gamma distribution parameters (shape, scale) [DEPRECATED]
                exponential_network: Network that predicts single exponential rate λ for importance sampling
            Training parameters:
                sigma_data, p_mean, p_std
            Sampling parameters:
                sigma_min, sigma_max, rho
                S_min, S_max, S_noise
        '''
        super(EDM, self).__init__()
        self.nn_model = nn_model.to(device)
        params = sum(p.numel() for p in nn_model.parameters() if p.requires_grad) / 1e6
        print(f"nn model # params: {params:.1f}")
        self.device = device

        def number_to_torch_device(value):
            return torch.tensor(value).to(device)

        self.sigma_data = number_to_torch_device(sigma_data)
        self.p_mean     = number_to_torch_device(p_mean)
        self.p_std      = number_to_torch_device(p_std)
        self.sigma_min  = number_to_torch_device(sigma_min)
        self.sigma_max  = number_to_torch_device(sigma_max)
        self.rho        = number_to_torch_device(rho)
        self.S_min      = number_to_torch_device(S_min)
        self.S_max      = number_to_torch_device(S_max)
        self.S_noise    = number_to_torch_device(S_noise)
        self.use_dsm = use_dsm

        self.s_noise_normalizer = s_noise_normalizer
        self.importance_sampler_net = importance_sampler_net
        self.mixing_noise_normalizer = mixing_noise_normalizer
        self.mixing_lam = mixing_lam

    def forward(self, x, use_dsm=False):
        ''' Training with weighted denoising loss.

            Args:
                x: The clean image tensor ranged in `[0, 1]`.
                reference_batch: Optional reference batch for stf_targets (already normalized to [-1, 1])
            Returns:
                The weighted MSE loss.
        '''
        x = normalize_to_neg_one_to_one(x)
        B, C, H, W = x.shape
        D = C * H * W
        rnd_normal = torch.randn((1, 1, 1, 1)).to(self.device)
        sigma = (rnd_normal * self.p_std + self.p_mean).exp().expand(x.shape[0], 1, 1, 1)
        sigma2 = sigma**2

        with autocast(dtype=torch.bfloat16):
            if use_dsm:
                total_loss = dsm_loss_x0(self.D_x, x, sigma2).mean()
            else:
                def f_theta(z): return (self.D_x(z, sigma) - z) / sigma2
                def f_theta_wo_sigma(z, in_sigma): return (self.D_x(z, in_sigma) - z) / in_sigma

                # mixing application loss
                mixing_loss = mixing_loss_fixed_sigma_edm(f_theta_wo_sigma, x, sigma)
                mixing_normalizer_weight = self.mixing_noise_normalizer(sigma)
                app_loss = mixing_loss / torch.exp(mixing_normalizer_weight) + mixing_normalizer_weight

                # main flux matching loss
                s_normalizer_weight = self.s_noise_normalizer(sigma)
                def f_theta(z): return (self.D_x(z, sigma) - z) / sigma2
                main_loss, t = flux_matching_loss(
                    f_theta, x, sigma2,
                    q=self.importance_sampler_net, return_t=True
                )
                loss_shape_det = (main_loss.detach()).mean() / torch.exp(s_normalizer_weight)
                is_loss = self.importance_sampler_net.reinforce_loss(
                    t, sigma, loss_shape_det.expand(B),
                )

                total_loss = (
                    main_loss / torch.exp(s_normalizer_weight) 
                    + s_normalizer_weight 
                    + is_loss
                    + self.mixing_lam * app_loss
                ).mean()

        return total_loss

    def edm_sample(self, n_sample, size, steps=50, eta=0.0, notqdm=False):
        ''' Sampling with EDM sampler. Actual NFE is `2 * steps - 1`.

            Args:
                n_sample: The batch size.
                size: The image shape (e.g. `(3, 32, 32)`).
                steps: The number of total timesteps.
                eta: controls stochasticity. Set `eta=0` for deterministic sampling.
            Returns:
                The sampled image tensor ranged in `[0, 1]`.
        '''
        S_min, S_max, S_noise = self.S_min, self.S_max, self.S_noise
        gamma_stochasticity = torch.tensor(np.sqrt(2) - 1) * eta # S_churn = (sqrt(2) - 1) * eta * steps

        times = self.sample_schedule(steps)
        time_pairs = list(zip(times[:-1], times[1:]))

        x_next = torch.randn(n_sample, *size).to(self.device).to(torch.float64) * times[0]
        
        for i, (t_cur, t_next) in enumerate(tqdm(time_pairs, disable=notqdm)): # 0, ..., N-1
            x_cur = x_next

            # Increase noise temporarily.
            gamma = gamma_stochasticity if S_min <= t_cur <= S_max else 0
            t_hat = t_cur + gamma * t_cur
            x_hat = x_cur + (t_hat ** 2 - t_cur ** 2).sqrt() * S_noise * torch.randn_like(x_cur)

            # Euler step.
            with autocast(dtype=torch.bfloat16):
                d_cur = self.pred_eps_(x_hat, t_hat)
            x_next = x_hat + (t_next - t_hat) * d_cur

            # Apply 2nd order correction.
            if i < steps - 1:
                with autocast(dtype=torch.bfloat16):
                    d_prime = self.pred_eps_(x_next, t_next)
                x_next = x_hat + (t_next - t_hat) * (0.5 * d_cur + 0.5 * d_prime)

        return unnormalize_to_zero_to_one(x_next)

    def pred_eps_(self, x, sigma, clip_x=True):
        denoised = self.D_x(x, sigma).to(torch.float64)
        # pixel-space clipping (optional)
        if clip_x:
            denoised = torch.clip(denoised, -1., 1.)
        eps = (x - denoised) / sigma
        return eps

    def D_x(self, x_noised, sigma):
        ''' Denoising with network preconditioning.

            Args:
                x_noised: The perturbed image tensor.
                sigma: The variance (noise level) tensor.
            Returns:
                The estimated denoised image tensor.
        '''
        x_noised = x_noised.to(torch.float32)
        sigma = sigma.to(torch.float32).squeeze()
        if len(sigma.shape) == 0: sigma = sigma.repeat(x_noised.shape[0])

        c_skip = self.sigma_data ** 2 / (sigma ** 2 + self.sigma_data ** 2)
        c_out = sigma * self.sigma_data / (sigma ** 2 + self.sigma_data ** 2).sqrt()
        c_in = 1 / (sigma ** 2 + self.sigma_data ** 2).sqrt()
        c_noise = sigma.log() / 4
        F_x = self.nn_model(c_in[:, None, None, None] * x_noised, c_noise.flatten())

        return c_skip[:, None, None, None] * x_noised + c_out[:, None, None, None] * F_x

    def sample_schedule(self, steps):
        ''' Make the variance schedule for EDM sampling.

            Args:
                steps: The number of total timesteps. Typically 18, 50 or 100.
            Returns:
                times: A decreasing tensor list such that
                    `times[0] == sigma_max`,
                    `times[steps-1] == sigma_min`, and
                    `times[steps] == 0`.
        '''
        sigma_min, sigma_max, rho = self.sigma_min, self.sigma_max, self.rho
        times = torch.arange(steps, dtype=torch.float64, device=self.device)
        times = (sigma_max ** (1 / rho) + times / (steps - 1) * (sigma_min ** (1 / rho) - sigma_max ** (1 / rho))) ** rho
        times = torch.cat([times, torch.zeros_like(times[:1])]) # t_N = 0
        return times